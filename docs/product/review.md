...
| D21 | **17 commits on `redo/architecture-scaffold` are not pushed to `origin`.** No off-machine copy of any of this work, and Claude Code on the web / mobile cannot see the branch at all until it is pushed | **high** | open | **Now.** This is the only item whose cost is total loss rather than inconvenience |
