...
| D22 | **Deploys are asymmetric: the web app builds on push, the API does not.** Pages is connected to GitHub, so a session with no shell can ship web changes. API deploys still need `gcloud builds submit` + `gcloud run deploy` from a terminal, so the same session cannot ship an API change at all | med | open | Before relying on a mobile/web session for API work. Needs a GitHub Actions workflow and a **deploy** service account key as a repo secret — not the runtime account, which must never get deploy rights |
