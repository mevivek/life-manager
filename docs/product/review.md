...
| D20 | **The app is reachable only while the laptop is awake.** A Cloudflare Tunnel serves `app.`/`api.mevivek.dev` from `localhost` — enough to verify ADR-0019 and install the PWA, but not hosting. Also means no other device, and no AI session without filesystem access, can reach a running instance | med | accepted | When the app needs to work with the laptop closed. Host choice is open — [ADR-0014](../decisions/0014-hosting-topology.md) amended |
...
| D22 | **No deploy path that works without a laptop terminal.** No `gcloud`/`wrangler`/`fly`/`docker` installed, and none should be required: the deploy must run in CI or no AI session without shell access can ship. Plan and browser prerequisites in [README.md](../../README.md) § Deploying; workflow files deliberately unwritten until credentials exist to test them | **high** | open | Before relying on a mobile/web session to ship anything, or before the app must survive a closed laptop (supersedes the workaround in D20) |
