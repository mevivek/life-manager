...
| D20 | **The WEB app is reachable only while the laptop is awake.** The API moved to Cloud Run on 2026-07-27 and `api.mevivek.dev` no longer depends on this machine ([ADR-0021](../decisions/0021-cloud-run-for-the-api.md)); the tunnel now serves only `app.mevivek.dev` | med | open | Cloudflare Pages for the web half — settings in [README.md](../../README.md) § Deploying |
