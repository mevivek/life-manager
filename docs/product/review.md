...
| D41 | **`DELETE /documents/:id` has no version precondition, so a stale delete can still destroy a newer edit.** `PATCH` got one in [ADR-0024](../decisions/0024-offline-writes-outbox.md); `DELETE` did not, because concurrent *edits* are the common case and a delete is usually deliberate and recent. But a delete queued in the outbox, replayed after the document was edited on another device, removes that edit with no conflict shown — the exact failure mode ADR-0024 exists to prevent | med | open | **Before the outbox can queue a delete.** Either add the precondition (a version query parameter, since DELETE has no body) or refuse to queue deletes offline at all |
