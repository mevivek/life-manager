...
| D25 | **The Cloud Build trigger holds an INLINE copy of `cloudbuild.deploy.yaml`,** because a webhook trigger has no attached repository to read it from. Editing the file changes nothing until the trigger is updated, and there is no error to indicate the stale copy ran | med | accepted | Every edit to that file: `gcloud builds triggers update webhook deploy-api-on-push --region=global --inline-config=cloudbuild.deploy.yaml --project=life-manager-01`. Closes properly only by connecting the repo to Cloud Build, which needs its GitHub App installed |
