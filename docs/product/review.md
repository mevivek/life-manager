...
| D62 | **The deploy guard skips a multi-commit push, because it only examines the tip commit.** `cloudbuild.deploy.yaml`'s `guard` step decides whether to deploy with `git diff --quiet HEAD~1 HEAD -- apps/api packages/shared …` — the tip commit against its parent. That is correct for a **merge commit**, and its own comment says so ("on a merge commit the first parent is the previous main, so the diff is exactly the merged work"). It is **wrong for a fast-forward**: a push of N commits has `_SHA` = the tip, so only the tip is examined and every API change behind it is skipped. **This caused a real outage on 2026-07-30.** A `--ff-only` merge landed 9 commits; the tip was web+docs, so the guard skipped — while three commits behind it had added `documents.thing_id` to the wire schema. The client then required a field the deployed API had never heard of, `lib/api.ts`'s Zod parse threw, and the Documents archive and the Now screen both failed to load. It only recovered because a later push happened to have an API change *as its tip commit* | high | **open** | **Two mitigations are already in place, and neither is the fix.** (1) `documentSchema.thing_id` is `.nullish().default(null)`, so the client now tolerates an older server — see its long note, and apply the same rule to every future response field. (2) Merge with **`--no-ff`**, which restores the assumption the guard is written against. **The real fix** is to diff against the push's before-SHA or the last deployed commit rather than `HEAD~1` — GitHub's push webhook sends `before`, so the trigger could pass it as a substitution. **But the config is not read from the repo**: the trigger holds its own inline copy, so editing `cloudbuild.deploy.yaml` changes nothing until the trigger is deleted and recreated (**D25**). Do those together, and verify by pushing two commits where only the first touches `apps/api` |
