...
| D59 | **The four thing-photo endpoints exist, are tested, and no client can call them.** M4 step 1 built `photos::presign-upload`, `::confirm`, `::presign-download`, the hero `PATCH` and the `DELETE` to [things.md](../domains/things.md) §5, with 22 tests. `apps/web/src/lib/api.ts`'s `api.things` has **no method for any of them**, so `ThingPhotos.tsx` still draws its honest dead end — *"These are on the record, but the app can't open a thing's photo yet"* — and there is no way to add a photo to a thing at all. The mirror image of the state Things was in before this milestone, one layer down | low | open | **The next time somebody wants a photo on a thing.** Small and mechanical: five methods on `api.things` copied from `api.files`, a presign → PUT → confirm hook, and turning the existing tiles into `presign-download` buttons. `ThingPhotos.tsx`'s header comment already lists exactly what to add. Do the outbox widening (things.md §10) in the same pass or not at all — both are "the client half the API session did not write" |
