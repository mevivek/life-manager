...
| [0020](0020-google-oauth-alongside-password.md) | Google sign-in | accepted — **email+password half superseded by 0035** | Google OAuth **alongside** email+password, with google as a trusted linking provider |
...
| [0026](0026-store-the-full-identifier.md) | Store the full document identifier, unencrypted | accepted — **detail-only rule superseded by 0027, display half by 0034** | **Reverses business rule 6.** Full value in `identifier`, plaintext — amends 0009's data-minimisation half, not its encryption half (invariant 7 stands). Its derived `identifier_last4` and its reveal-to-read display are **replaced by 0034**; the storage decision stands |
| [0027](0027-identifier-in-the-list-response.md) | The full identifier is returned in the list response | accepted — **display half superseded by 0034** | **Supersedes 0026's detail-only rule.** `identifier` on `documentSchema`, so the archive shows and copies a number with no detail round-trip. Cost accepted and named: the persisted cache holds every number on the device (**D47**). Its per-row Show and page-wide header toggle are **replaced by 0034** |
...
| [0032](0032-one-library-tab.md) | Documents and Things share one tab | accepted — **§ Deviation superseded by 0033** | **Supersedes 0031's fourth tab.** The bar is `Now · Everything · You`; `/library` holds both collections with `All / Documents / Things` scope pills, `All` interleaving them by the date that bites first. Not 0029's switcher returning — that navigated between two screens and could never render `All`. Add now asks which track (a fork, **not** a seventh wizard step). Deviates from the comp by keeping the per-domain filters, and says why |
| [0033](0033-handoff-5-the-rest.md) | The rest of handoff 5 | accepted | **Supersedes 0032 § Deviation**: the filter chips come off, the query parameters stay, and a `Clear` beside the count replaces them. One row builder so a row cannot change shape with the scope. A vehicle's registration is **not masked** — it is painted on the outside of the car. A row-level *Add scan*. You counts both domains and can turn reminders off. **Declines handoff 5's Google-only sign-in**, because [0020](0020-google-oauth-alongside-password.md) rejected exactly that on account-recovery grounds |
| [0034](0034-people-is-a-directory.md) | People is a directory of names, not a foreign key | accepted | A `people` table nothing points at. `documents.holder` stays a **string**, and a rename is a bulk update over it in one transaction. Forced by the comp's own two sentences — *"removing them leaves the documents alone"* and *"the records **keep the name**"* — which a FK cannot express on delete without nulling, blocking or cascading. No existing response shape changes, so no deploy-ordering hazard (D54). Revisit when a person acquires facts of their own |
| [0035](0035-google-only-sign-in.md) | Google-only sign-in, gated on the server having Google | accepted | **Supersedes 0020's *email+password stays*.** The screens are the comp's one button, and `GET /api/v1/auth/providers` says whether that can work — derived from the same env check that registers the provider, so it cannot claim a capability the auth instance lacks. Falls back to the password form when Google is absent **or the probe fails**, because `GOOGLE_CLIENT_*` are optional and nothing in cloudbuild sets them. `emailAndPassword` stays enabled server-side as the recovery path. **The lost-Google-account risk is accepted, not solved — revisit before the vault ships** |
| [0036](0036-numbers-shown-by-default.md) | Numbers are shown by default; masking is one device preference | accepted | **Supersedes the display half of 0026 and 0027.** A number was masked everywhere and revealed by three separate controls, guarding nothing — the value is in the payload before any tap, as 0027 itself says. Now shown by default, with `feel.numbers` (You screen, device-scoped) as the one opt-in mask across **both** domains. The library's page-wide toggle is deleted, and the derived `identifier_last4` / `serial_last4` columns are **dropped** — the client cuts the tail. Stale-bundle cost named: **D86** |
...
it is not encrypted · 0034 why it is also SHOWN, and where the one mask preference lives · 0029 the
second status ladder · 0030 why capture has steps
